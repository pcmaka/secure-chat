...
var nsiRandomGenerator = Cc["@mozilla.org/security/random-generator;1"].getService(Ci.nsIRandomGenerator);

if (typeof window === 'undefined') {
  //nodeCrypto = require('./chance');
}
...
module.exports.rnd = {
  ...
  getRandomValues: function(buf) {
    if (typeof window !== 'undefined' && window.crypto) {
	  ...
	  var bytes = nsiRandomGenerator.generateRandomBytes(buf.length);
	  console.log(bytes);
	  buf.set(bytes);
      if(!bytes)
	  {
